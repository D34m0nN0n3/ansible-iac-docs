...
.. hint::  **SSH ключь передается как шифрованный секрет. Прароль для каждого сервера генерится случайным образом.**
.. attention:: **Для получения пароля от файла с секретом необходимо обратится в отдел УдЦ ГВЦ.**
